...
let harpischord = [];
let piano = [];
let instrument1 = [0,1,2,3,4,5,6,7];

let socket = io('/instrument2');
...
function preload() {
  soundFormats('wav');
  for(let i = 0; i<8; i++){
    harpischord.push(loadSound('data/'+i));
    piano.push(loadSound('data/p'+i));
  }
...
function draw() {
  ...
  if (timeNow > timeStep) {
    ...
    if(currentStep < segmentCount-1){
      ...
      harpischord[steps[currentStep].state].play();
      piano[instrument1[currentStep]].play();
...
function updateOther(upd){
  ...
  instrument1[upd.step] = upd.change;
}
